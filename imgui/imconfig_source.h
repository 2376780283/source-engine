...
// 这些傻逼接口需要跨平台 imgui.h
IMGUI_API ImFileHandle	ImSourceFileOpen( const char *filename, const char *mode );
IMGUI_API bool			ImSourceFileClose( ImFileHandle file );
IMGUI_API uint64		ImSourceFileGetSize( ImFileHandle file );
IMGUI_API uint64		ImSourcrFileRead( void *data, uint64 size, uint64 count, ImFileHandle file );
IMGUI_API uint64		ImSourceFileWrite( const void *data, uint64 size, uint64 count, ImFileHandle file );
