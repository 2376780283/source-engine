...
class GamepadUIKeyButton : public GamepadUIOptionButton {
   public:
    ...
    GamepadUIKeyButton(const char *pszBinding, vgui::Panel *pParent, vgui::Panel *pActionSignalTarget, const char *pSchemeFile, const char *pCommand, const char *pText, const char *pDescription)
        : BaseClass(pParent, pActionSignalTarget, pSchemeFile, pCommand, pText, pDescription), m_szBinding(pszBinding) {
    }
...
class GamepadUIConvarButton : public GamepadUIOptionButton {
   public:
    ...
    GamepadUIConvarButton(const char *pszCvar, const char *pszCvarDepends, bool bInstantApply, vgui::Panel *pParent, vgui::Panel *pActionSignalTarget, const char *pSchemeFile, const char *pCommand, const char *pText, const char *pDescription)
        : BaseClass(pParent, pActionSignalTarget, pSchemeFile, pCommand, pText, pDescription), m_cvar(pszCvar), m_szDependentCVar(pszCvarDepends), m_bInstantApply(bInstantApply) {
    }
...
class GamepadUIWheelyWheel : public GamepadUIConvarButton {
   public:
    ...
    GamepadUIWheelyWheel(const char *pszCvar, const char *pszCvarDepends, bool bInstantApply, bool bSignOnly, vgui::Panel *pParent, vgui::Panel *pActionSignalTarget, const char *pSchemeFile, const char *pCommand, const char *pText, const char *pDescription)
        : BaseClass(pszCvar, pszCvarDepends, bInstantApply, pParent, pActionSignalTarget, pSchemeFile, pCommand, pText, pDescription), m_bSignOnly(bSignOnly) {
    }
    ...
    void ShowWarning(int nIndex, bool bIncrement) {
        new GamepadUIGenericConfirmationPanel(GamepadUIOptionsPanel::GetInstance(), "OptionWarning", GamepadUIString("#GameUI_Confirm").String(), m_DangerousOptionsText[nIndex].String(), [this, bIncrement]() { bIncrement ? this->IncrementValueInner() : this->DecrementValueInner(); }, true, true);
    }
...
class GamepadUISkillySkill : public GamepadUIOptionButton {
   public:
    ...
    GamepadUISkillySkill(vgui::Panel *pParent, vgui::Panel *pActionSignalTarget, const char *pSchemeFile, const char *pCommand, const char *pText, const char *pDescription, const char *pImage, int nSkill)
        : BaseClass(pParent, pActionSignalTarget, pSchemeFile, pCommand, pText, pDescription), m_Image(pImage), m_nSkill(nSkill) {
    }
...
void GamepadUIOptionsPanel::LayoutCurrentTab() {
    ...
    int nCenterStartX = (nParentW - nTotalTabsWidth) / 2;
    int x = nCenterStartX;
    int y = 0;
    ...
    for (GamepadUIOptionButton *pButton : m_Tabs[nActiveTab].pButtons) {
        ...
        int buttonY = y;
        int buttonX = nOptionsCenterStartX;
    ...
    int yMax = 0;
    {
        ...
        float flScreenRatioW, flScreenRatioH;
        GamepadUI::GetInstance().GetScreenRatio(flScreenRatioW, flScreenRatioH);

        float flScrollBarX = nOptionsCenterStartX + nMaxButtonWidth + (20 * flScreenRatioW);

        if (flScrollBarX + m_pScrollBar->GetWide() > nParentW) {
            flScrollBarX = nParentW - m_pScrollBar->GetWide() - 10;
        }

        m_pScrollBar->InitScrollBar(&m_Tabs[nActiveTab].ScrollState,
                                    flScrollBarX, m_flScrollBarOffsetY);
...
void GamepadUIOptionsPanel::OnCommand(char const *pCommand) {
    if (!V_strcmp(pCommand, "action_back")) {
        ...
        new GamepadUIGenericConfirmationPanel(GamepadUIOptionsPanel::GetInstance(), "UseDefaultsConfirm", GamepadUIString("#GameUI_KeyboardSettings").String(), GamepadUIString("#GameUI_KeyboardSettingsText").String(), []() { GamepadUI::GetInstance().GetEngineClient()->ClientCmd_Unrestricted("exec config_default.cfg\n_gamepadui_resetkeys\n"); }, false, true);
        ...
        V_snwprintf(wszBuf, 4096, L"%S\n\n%S\n\n%S", bink.String(), miles.String(), voice.String());
#endif
        new GamepadUIGenericConfirmationPanel(GamepadUIOptionsPanel::GetInstance(), "TechCredits", title.String(), wszBuf, []() {}, true, false);
