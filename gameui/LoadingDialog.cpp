...
// --------------------------------------------------------------------------------
// Loading ProgressBar Color list for loading dialog
// --------------------------------------------------------------------------------
struct ProgressBarColor_t {
    const char *szKey;
    unsigned char r, g, b, a;
};

static const ProgressBarColor_t g_ProgressBarColors[] = {
    {"portal", 49, 185, 224, 224},
    {"entropyzero2", 255, 46, 0, 224},
    {"hl2", 201, 100, 0, 224},
};
...
CLoadingDialog::CLoadingDialog(vgui::Panel *parent) : Frame(parent, "LoadingDialog") {
    ...
    // center the loading dialog, unless we have another dialog to show in the
    // background
    ...
    if (IsGamepadUI()) {
        ...
        Color color = pScheme->GetColor("SteamDeckSpinner", Color(0, 0, 0, 0));

        if (color == Color(0, 0, 0, 0)) {
            const char *pGame = CommandLine()->ParmValue("-game", "hl2");
            color.SetColor(201, 100, 0, 244); // 默认橙

            for (const auto &item : g_ProgressBarColors) {
                if (Q_stristr(pGame, item.szKey)) {
                    color.SetColor(item.r, item.g, item.b, item.a);
                    break;
                }
            }
        }
        cProgress->SetGainColor(color);
        cProgress->SetLossColor(color);
    } else {
...
CLoadingDialog::~CLoadingDialog() {
    if (input()->GetAppModalSurface() == GetVPanel()) { vgui::surface()->RestrictPaintToSinglePanel(NULL); }
}
...
void CLoadingDialog::PaintBackground() {
    ...
    if (ModInfo().IsSinglePlayerOnly()) { DrawBox(x, y, barWide, barTall, Color(0, 0, 0, 255), 1.0f); }
...
void CLoadingDialog::Open() {
    if (!m_bConsoleStyle) { SetTitle("#GameUI_Loading", true); }
    ...
    if (!m_bConsoleStyle) {
        ...
        if (!ModInfo().IsSinglePlayerOnly()) { m_pInfoLabel->SetVisible(true); }
...
void CLoadingDialog::HideOtherDialogs(bool bHide) {
    if (bHide) {
        if (GameUI().HasLoadingBackgroundDialog()) {
            // if we have a loading background dialog, hide any other dialogs by
            // moving the full-screen background dialog to the front, then moving
            // ourselves in front of it
            GameUI().ShowLoadingBackgroundDialog();
            vgui::ipanel()->MoveToFront(GetVPanel());
            vgui::input()->SetAppModalSurface(GetVPanel());
        } else {
            // if there is no loading background dialog, use VGUI paint restrictions
            // to hide other dialogs
...
void CLoadingDialog::DisplayGenericError(const char *failureReason, const char *extendedReason) {
    ...
    // then it'll continue closing when we don't want it to.
    Activate();
...
bool CLoadingDialog::SetShowProgressText(bool show) {
    if (m_bConsoleStyle) { return false; }
...
void CLoadingDialog::OnThink() {
    ...
    if (!m_bConsoleStyle && m_bShowingSecondaryProgress) {
        ...
        } else if (ProgressBar::ConstructTimeRemainingString(unicode, sizeof(unicode), m_flSecondaryProgressStartTime, (float)system()->GetFrameTime(),
                                                             m_flSecondaryProgress, m_flLastSecondaryProgressUpdateTime, true)) {
...
void CLoadingDialog::PerformLayout() {
    if (m_bConsoleStyle) {
        ...
        // if we're not supposed to be centered, move ourselves to the lower right
...
bool CLoadingDialog::SetProgressPoint(float fraction) {
    ...
    if (!m_bShowingVACInfo && gameuifuncs->IsConnectedToVACSecureServer()) { SetupControlSettings(false); }
...
void CLoadingDialog::SetSecondaryProgress(float progress) {
    ...
    if (!m_bShowingSecondaryProgress && progress > 0.99f) return;
