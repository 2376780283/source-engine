...
#undef MessageBox // Windows helpfully #define's this to MessageBoxA, we're
                  // using vgui::MessageBox
...
//-----------------------------------------------------------------------------
// Purpose: hack function to give the module loader access to the main panel
// handle
...
void CGameMenuItem::ApplySchemeSettings(IScheme *pScheme) {
    ...
    if (m_bRightAligned) { SetContentAlignment(Label::a_east); }
}
...
void CGameMenuItem::PaintBackground() {
    if (!GameUI().IsConsoleUI()) {
        ...
        if (!IsArmed() || !IsVisible() || GetParent()->GetAlpha() < 32) return;
...
class CGameMenu : public vgui::Menu {
  public:
    ...
    virtual void ApplySchemeSettings(IScheme *pScheme) {
        ...
        if (IsProportional()) height = scheme()->GetProportionalScaledValue(height);
    ...
    virtual void SetVisible(bool state) {
        ...
        // move us to the back instead of going invisible
        if (!state) { ipanel()->MoveToBack(GetVPanel()); }
    }
    ...
    virtual void SetMenuItemBlinkingState(const char *itemName, bool state) {
        for (int i = 0; i < GetChildCount(); i++) {
            ...
            if (menuItem) {
                if (Q_strcmp(menuItem->GetCommand()->GetString("command", ""), itemName) == 0) { menuItem->SetBlink(state); }
            }
        }
        InvalidateLayout();
    }
    ...
    virtual void OnSetFocus() {
        if (m_hMainMenuOverridePanel) {
            ...
            if (pMainMenu) { pMainMenu->PerformLayout(); }
        }
    ...
    virtual void OnKeyCodePressed(KeyCode code) {
        if (IsX360()) {
            ...
            if (code == KEY_XBUTTON_B || code == KEY_XBUTTON_START) {
                if (GameUI().IsInLevel()) { GetParent()->OnCommand("ResumeGame"); }
                return;
            }
        }
    ...
    virtual void OnKillFocus() {
        ...
        if (m_hMainMenuOverridePanel) {
            // force us to the rear when we lose focus (so it looks like the menu is
    ...
    void ShowFooter(bool bShow) {
        if (m_pConsoleFooter) { m_pConsoleFooter->SetVisible(bShow); }
    }
    ...
    void UpdateMenuItemState(bool isInGame, bool isMultiplayer, bool isInReplay, bool isVREnabled, bool isVRActive) {
        ...
        for (int i = 0; i < GetChildCount(); i++) {
            ...
            if (menuItem) {
                ...
                if (!isInGame && kv->GetInt("OnlyInGame")) { shouldBeVisible = false; }
                ...
                if (isInReplay && !kv->GetInt("OnlyInReplay")) { shouldBeVisible = false; }
        ...
        if (!isInGame) {
            ...
            for (int j = 0; j < GetChildCount() - 2; j++) { MoveMenuItem(j, j + 1); }
        } else {
            ...
            for (int i = 0; i < GetChildCount(); i++) {
                for (int j = i; j < GetChildCount() - 2; j++) {
                    ...
                    if (kv1->GetInt("InGameOrder") > kv2->GetInt("InGameOrder")) MoveMenuItem(iID2, iID1);
                }
            }
        }
        ...
        if (m_pConsoleFooter) {
            ...
            if (!m_pConsoleFooter->GetHelpName() || V_stricmp(pHelpName, m_pConsoleFooter->GetHelpName())) {
                ...
                if (isInGame) { m_pConsoleFooter->AddNewButtonLabel("#GameUI_Close", "#GameUI_Icons_B_BUTTON"); }
            }
        }
    }
...
void CGameMenu::OnCursorEnteredMenuItem(VPANEL menuItem) {
    ...
    if (!pCommand->GetFirstSubKey()) return;
...
CBasePanel::CBasePanel() : Panel(NULL, "BaseGameUIPanel") {
    ...
    //	m_pGameMenuButtons.AddToTail( CreateMenuButton( this, "GameMenuButton",
    // ModInfo().GetGameTitle() ) ); 	m_pGameMenuButtons.AddToTail(
    // CreateMenuButton( this, "GameMenuButton2", ModInfo().GetGameTitle2() ) );
    if (!IsSteamDeck()) {
    ...
    if (SteamClient()) {
        ...
        if (pUtils) { bSteamCommunityFriendsVersion = true; }
    ...
    // Bonus maps menu blinks if something has been unlocked since the player last
    // opened the menu This is saved as persistant data, and here is where we
    // check for that
...
//-----------------------------------------------------------------------------
// Purpose: Xbox 360 - Get the console UI keyvalues to pass to
// LoadControlSettings()
//-----------------------------------------------------------------------------
...
static int CC_GameMenuCompletionFunc(char const *partial, char commands[COMMAND_COMPLETION_MAXITEMS][COMMAND_COMPLETION_ITEM_LENGTH]) {
    ...
    if (Q_strstr(partial, cmdname)) { substring = (char *)partial + strlen(cmdname) + 1; }
    ...
    for (i = symbols.FirstInorder(); i != symbols.InvalidIndex(); i = symbols.NextInorder(i)) {
        ...
        Q_snprintf(commands[slot++], COMMAND_COMPLETION_ITEM_LENGTH, "%s %s", cmdname, buf);
    }
...
void CBasePanel::PaintBackground() {
    if (!GameUI().IsInLevel() || g_hLoadingDialog.Get() || m_ExitingFrameCount) {
        // not in the game or loading dialog active or exiting, draw the ui
        // background
        DrawBackgroundImage();
    } else if (IsX360()) {
...
void CBasePanel::UpdateBackgroundState() {
    if (m_ExitingFrameCount) {
        ...
        // level loading is truly completed when the progress bar is gone, then
        // transition to main menu
        if (IsPC() || (IsX360() && !g_hLoadingDialog.Get())) { SetBackgroundRenderState(BACKGROUND_MAINMENU); }
    ...
    if (GameUI().IsConsoleUI()) {
        if (!m_ExitingFrameCount && !m_bLevelLoading && !g_hLoadingDialog.Get() && GameUI().IsInLevel()) {
            // paused
            if (m_flBackgroundFillAlpha == 0.0f) m_flBackgroundFillAlpha = 120.0f;
        } else {
    ...
    if (!m_bPlatformMenuInitialized) return;
    ...
    for (i = 0; i < GetChildCount(); ++i) {
        ...
        if (child && ipanel()->IsVisible(child) && ipanel()->IsPopup(child) && child != m_pGameMenu->GetVPanel()) { bHaveActiveDialogs = true; }
    }
    // see if the base gameui panel has dialogs hanging off it (engine stuff,
    // console, bug reporter)
    VPANEL parent = GetVParent();
    for (i = 0; i < ipanel()->GetChildCount(parent); ++i) {
        VPANEL child = ipanel()->GetChild(parent, i);
        if (child && ipanel()->IsVisible(child) && ipanel()->IsPopup(child) && child != GetVPanel()) { bHaveActiveDialogs = true; }
    }
    ...
    if (m_bHaveDarkenedBackground != bNeedDarkenedBackground) {
        ...
        vgui::GetAnimationController()->RunAnimationCommand(this, "m_flBackgroundFillAlpha", targetAlpha, 0.0f, duration,
                                                            AnimationController::INTERPOLATOR_LINEAR);
    }

    // check to see if the game title should be dimmed
    // don't transition on level change
    if (m_bLevelLoading) return;
    ...
    if (m_bHaveDarkenedTitleText != bNeedDarkenedTitleText || m_bForceTitleTextUpdate) {
        ...
        if (m_pGameLogo) {
            vgui::GetAnimationController()->RunAnimationCommand(m_pGameLogo, "alpha", targetTitleAlpha, 0.0f, duration,
                                                                AnimationController::INTERPOLATOR_LINEAR);
        }
        ...
        for (i = 0; i < m_pGameMenuButtons.Count(); ++i) {
            vgui::GetAnimationController()->RunAnimationCommand(m_pGameMenuButtons[i], "alpha", targetTitleAlpha, 0.0f, duration,
                                                                AnimationController::INTERPOLATOR_LINEAR);
        }
...
void CBasePanel::SetBackgroundRenderState(EBackgroundState state) {
    if (state == m_eBackgroundState) { return; }
    ...
    if (m_eBackgroundState == BACKGROUND_INITIAL && (state == BACKGROUND_DISCONNECTED || state == BACKGROUND_MAINMENU)) {
        ...
        if (dev_loadtime_mainmenu) { dev_loadtime_mainmenu->SetValue(frametime); }
    }
    ...
    if (state == BACKGROUND_EXITING) {
        ...
        if (GameUI().IsConsoleUI()) { RunAnimationWithCallback(this, "InstantHideMainMenu", new KeyValues("LoadMap")); }
...
void CBasePanel::StartExitingProcess() {
    ...
    CMatchmakingBasePanel *pPanel = GetMatchmakingBasePanel();
    if (pPanel) { pPanel->CloseAllDialogs(false); }
...
void CBasePanel::OnSizeChanged(int newWide, int newTall) {
    ...
    if (m_hMatchmakingBasePanel.Get()) { m_hMatchmakingBasePanel->SetBounds(0, 0, newWide, newTall); }
}
...
void CBasePanel::OnLevelLoadingStarted() {
    ...
    if (m_hMatchmakingBasePanel.Get()) { m_hMatchmakingBasePanel->OnCommand("LevelLoadingStarted"); }
...
void CBasePanel::OnLevelLoadingFinished() {
    ...
    if (m_hMatchmakingBasePanel.Get()) { m_hMatchmakingBasePanel->OnCommand("LevelLoadingFinished"); }
}
...
void CBasePanel::DrawBackgroundImage() {
    if (IsX360() && m_bCopyFrameBuffer) {
        // force the engine to do an image capture ONCE into this image's render
        // target
    ...
    if (IsX360()) {
        if (m_ExitingFrameCount) {
            ...
            // the render target image must be opaque, the alpha channel contents are
            // unknown it is strictly an opaque background image and never used as an
            // overlay
    ...
    // 360 always use the progress bar, TCR Requirement, and never this loading
    // plaque
    if (IsPC() && (m_bRenderingBackgroundTransition || m_eBackgroundState == BACKGROUND_LOADING)) {
        ...
        if (IsSteamDeck()) {
            ...
            surface()->DrawGetTextureSize(m_iLoadingSpinnerImageID, twide,
                                          ttall); // now use twide and ttall for spinner
            ...
            surface()->DrawSetTextureFrame(m_iLoadingSpinnerImageID, ((int)m_fLoadingSpinnerFrame) % surface()->GetTextureNumFrames(m_iLoadingSpinnerImageID),
                                           &nFrameCache);
    ...
    if (m_bFadingInMenus) {
        if (GameUI().IsConsoleUI()) {
            ...
            if (alpha == 255) { m_bFadingInMenus = false; }
        }
    }
}
...
void CBasePanel::CreateGameMenu() {
    ...
    if (datafile->LoadFromFile(g_pFullFileSystem, "Resource/GameMenu.res")) { m_pGameMenu = RecursiveLoadGameMenu(datafile); }
...
#ifdef _X360
    // On 360 if we have a storage device at this point and try to read the bonus
    // data it can't find the bonus file!
    return;
#endif

    if (BonusMapsDatabase()->GetBlink()) {
        if (GameUI().IsConsoleUI())
            SetMenuItemBlinkingState("OpenNewGameDialog",
                                     true); // Consoles integrate bonus maps menu into the new game menu
        else
...
void CBasePanel::UpdateGameMenus() {
    ...
    if (m_hMainMenuOverridePanel) { vgui::ivgui()->PostMessage(m_hMainMenuOverridePanel, new KeyValues("UpdateMenu"), NULL); }
...
CGameMenu *CBasePanel::RecursiveLoadGameMenu(KeyValues *datafile) {
    ...
    if (CommandLine()->CheckParm("-console")) { menu->AddMenuItem("Console", "CONSOLE", "OpenConsole", this); }
    ...
    for (KeyValues *dat = datafile->GetFirstSubKey(); dat != NULL; dat = dat->GetNextKey()) {
        ...
        if (cmd && Q_strcmp(cmd, "OpenServerBrowser") == 0) bFoundServerBrowser = true;
    }
    ...
    for (KeyValues *dat = datafile->GetFirstSubKey(); dat != NULL; dat = dat->GetNextKey()) {
        ...
        if (cmd && (!Q_stricmp(cmd, "OpenFriendsDialog") || !Q_stricmp(cmd, "engine bug"))) continue;
...
void CBasePanel::RunFrame() {
        ...
        if (IsX360() && m_ExitingFrameCount && engine->Time() >= m_flTransitionEndTime) {
            if (m_ExitingFrameCount > 1) {
                ...
                if (m_ExitingFrameCount == 1) {
                    ...
                    // If we kicked off this event from an invite, we need to properly
                    // setup the restart to account for that
    ...
    if (!m_bPlatformMenuInitialized) {
        ...
        if (IsX360() || g_VModuleLoader.IsPlatformReady()) { m_bPlatformMenuInitialized = true; }
    }
...
#if defined(_X360)
    // For all other users logged into this console (not primary), set to idle to
    // satisfy cert
    for (uint i = 0; i < XUSER_MAX_COUNT; ++i) {
        ...
        if (State != eXUserSigninState_NotSignedIn) {
            if (i != XBX_GetPrimaryUserId()) {
                // Set rich presence as 'idle' for users logged in that can't
                // participate in orange box.
                if (!xboxsystem->UserSetContext(i, X_CONTEXT_PRESENCE, CONTEXT_PRESENCE_IDLE, true)) { Warning("BasePanel: UserSetContext failed.\n"); }
            }
        }
    }

    if (!GameUI().IsInLevel()) {
        if (!xboxsystem->UserSetContext(XBX_GetPrimaryUserId(), CONTEXT_GAME, m_iGameID, true)) { Warning("BasePanel: UserSetContext failed.\n"); }
...
void CBasePanel::PerformLayout() {
    ...
    if (idealMenuY + menuTall + m_iGameMenuInset > tall) { idealMenuY = tall - menuTall - m_iGameMenuInset; }
    ...
    for (int i = 0; i < m_pGameMenuButtons.Count(); ++i) {
        ...
        // vgui::surface()->GetTextSize( m_pGameMenuButtons[i]->GetFont(),
        // ModInfo().GetGameTitle(), textWide, textTall );
...
void CBasePanel::ApplySchemeSettings(IScheme *pScheme) {
    ...
    if (pClientScheme) {
        ...
        for (i = 0; i < m_pGameMenuButtons.Count(); ++i) {
            ...
            if (GameUI().IsConsoleUI()) m_iGameTitlePos[i].x += MAIN_MENU_INDENT_X360;
    ...
    if (m_iBackgroundImageID == -1) { m_iBackgroundImageID = surface()->CreateNewTextureID(); }
    ...
    if (IsX360()) {
        ...
        if (m_iProductImageID == -1) { m_iProductImageID = surface()->CreateNewTextureID(); }
...
void CBasePanel::OnGameUIActivated() {
    ...
    if (!m_bEverActivated) {
        // Layout the first time to avoid focus issues (setting menus visible will
        // grab focus)
        UpdateGameMenus();
        ...
        if (XBX_GetPrimaryUserId() != XBX_INVALID_USER_ID && XBX_GetStorageDeviceId() != XBX_INVALID_STORAGE_ID &&
    ...
    if (GameUI().IsConsoleUI()) { ArmFirstMenuItem(); }
    if (GameUI().IsInLevel()) {
        ...
        if (m_hAchievementsDialog.Get()) {
            // Achievement dialog refreshes it's data if the player looks at the pause
            // menu
            m_hAchievementsDialog->OnCommand("OnGameUIActivated");
        }
    } else // not the pause menu, update presence
    {
        if (IsX360()) { UpdateRichPresenceInfo(); }
    }
}
...
void CBasePanel::RunMenuCommand(const char *command) {
    if (!Q_stricmp(command, "OpenGameMenu")) {
        if (m_pGameMenu) { PostMessage(m_pGameMenu, new KeyValues("Command", "command", "Open")); }
        ...
        if (GameUI().IsConsoleUI()) { OnOpenControllerDialog(); }
        ...
        if (IsPC()) {
            ...
            if (!steamapicontext->SteamUser() || !steamapicontext->SteamUser()->BLoggedOn()) {
                vgui::MessageBox *pMessageBox =
                    new vgui::MessageBox("#GameUI_Achievements_SteamRequired_Title", "#GameUI_Achievements_SteamRequired_Message", this);
                pMessageBox->DoModal();
                return;
            }
#endif
        ...
        if (IsPC()) {
            /*           if ( !steamapicontext->SteamUser() ||
               !steamapicontext->SteamUser()->BLoggedOn() )
                       {
                           vgui::MessageBox *pMessageBox = new
               vgui::MessageBox("#GameUI_Achievements_SteamRequired_Title",
               "#GameUI_Achievements_SteamRequired_Message", this );
                           pMessageBox->DoModal();
                           return;
                       }*/
    ...
    //=============================================================================
        ...
        if (IsX360()) {
            if (m_hAchievementsDialog.Get()) { m_hAchievementsDialog->Close(); }
        }
        ...
        if (IsX360()) {
            ...
            if (GameClientExports()) { GameClientExports()->ShutdownAchievementPanel(); }
        ...
        if (strlen(engineCMD) > 0) { engine->ClientCmd_Unrestricted(const_cast<char *>(engineCMD)); }
        ...
        // The user doesn't care, so re-send the command they wanted and mark that
        // we want to skip checking
        m_bUserRefusedSignIn = true;
        if (m_strPostPromptCommand.IsEmpty() == false) { OnCommand(m_strPostPromptCommand); }
        ...
        if (!IsX360()) {
            ...
            // registry key and exec it in order to re-load the game in the proper
            // language
...
void CBasePanel::RunQueuedCommands() {
    for (int i = 0; i < m_CommandQueue.Count(); ++i) { OnCommand(m_CommandQueue[i]); }
    ClearQueuedCommands();
}
...
//-----------------------------------------------------------------------------
// Purpose: Whether this command should cause us to prompt the user if they're
// not signed in and do not have a storage device
//-----------------------------------------------------------------------------
bool CBasePanel::IsPromptableCommand(const char *command) {
    // Blech!
    if (!Q_stricmp(command, "OpenNewGameDialog") || !Q_stricmp(command, "OpenLoadGameDialog") || !Q_stricmp(command, "OpenSaveGameDialog") ||
        !Q_stricmp(command, "OpenBonusMapsDialog") || !Q_stricmp(command, "OpenOptionsDialog") || !Q_stricmp(command, "OpenControllerDialog") ||
        !Q_stricmp(command, "OpenLoadCommentaryDialog") || !Q_stricmp(command, "Extra_manager") ||
        !Q_stricmp(command, "OpenLoadSingleplayerCommentaryDialog") || !Q_stricmp(command, "OpenAchievementsDialog") ||
...
static uintp PanelJobWrapperFn(void *pvContext) {
    ...
    if (flElapsedTime < pAsync->m_flLeastExecuteTime) { ThreadSleep((pAsync->m_flLeastExecuteTime - flElapsedTime) * 1000); }
...
bool CBasePanel::CommandRequiresSignIn(const char *command) {
    ...
    if (!Q_stricmp(command, "OpenAchievementsDialog") ||
        ...
        !Q_stricmp(command, "OpenLoadGameDialog") || !Q_stricmp(command, "OpenSaveGameDialog") || !Q_stricmp(command, "OpenRankingsDialog"))
        return true;
...
bool CBasePanel::CommandRequiresStorageDevice(const char *command) {
    ...
    if (!Q_stricmp(command, "OpenSaveGameDialog") || !Q_stricmp(command, "OpenLoadGameDialog")) return true;

    return false;
}

//-----------------------------------------------------------------------------
// Purpose: Whether the command requires the user to have a valid profile
// selected
//-----------------------------------------------------------------------------
bool CBasePanel::CommandRespectsSignInDenied(const char *command) {
    // Anything which touches the user profile must prompt
    if (!Q_stricmp(command, "OpenOptionsDialog") || !Q_stricmp(command, "OpenControllerDialog")) return true;

    return false;
}

//-----------------------------------------------------------------------------
// Purpose: A storage device has been connected, update our settings and
// anything else
//-----------------------------------------------------------------------------
...
void CAsyncCtxOnDeviceAttached::ExecuteAsync() {
    ...
    if (m_ContainerOpenResult != ERROR_SUCCESS) return;
...
void CBasePanel::OnCompletedAsyncDeviceAttached(CAsyncCtxOnDeviceAttached *job) {
    ...
    if (m_hSaveGameDialog_Xbox.Get()) { m_hSaveGameDialog_Xbox->OnCommand("RefreshSaveGames"); }
    if (m_hLoadGameDialog_Xbox.Get()) { m_hLoadGameDialog_Xbox->OnCommand("RefreshSaveGames"); }
    if (m_hOptionsDialog_Xbox.Get()) { m_hOptionsDialog_Xbox->OnCommand("RefreshOptions"); }
    if (m_pStorageDeviceValidatedNotify) {
...
bool CBasePanel::ValidateStorageDevice(int *pStorageDeviceValidated) {
    ...
    if (pStorageDeviceValidated) {
        if (HandleStorageDeviceRequest("")) return true;
...
bool CBasePanel::HandleSignInRequest(const char *command) {
#ifdef _X360
    // If we have a post-prompt command, we're coming back into the call from that
    // prompt
    ...
    if (bQueuedCall) {
        ...
        // we're done
        if (m_bUserRefusedSignIn && CommandRespectsSignInDenied(command)) return true;
...
bool CBasePanel::HandleStorageDeviceRequest(const char *command) {
    ...
    if (XBX_GetStorageDeviceId() != XBX_INVALID_STORAGE_ID && XBX_GetStorageDeviceId() != XBX_STORAGE_DECLINED) return true;
    ...
    if (bQueuedCall && m_bStorageBladeShown) {
        // User has declined
        if (m_bUserRefusedStorageDevice) return true;
        ...
        // If the user refused the sign-in and we respect that on this command,
        // we're done
        if (m_bUserRefusedStorageDevice && CommandRespectsSignInDenied(command)) return true;

#if 0  // This attempts to find user data, but may not be cert-worthy even though
       // it's a bit nicer for the user Attempt to automatically find a device
		DWORD nFoundDevice = xboxsystem->DiscoverUserData( XBX_GetPrimaryUserId(), COM_GetModDirectory() );
		if ( nFoundDevice != XBX_INVALID_STORAGE_ID )
		{
			// Take this device
...
//-----------------------------------------------------------------------------
// Purpose: Issue our queued command to either the base panel or the matchmaking
// panel
//-----------------------------------------------------------------------------
void CBasePanel::IssuePostPromptCommand(void) {
    ...
    if (m_strPostPromptCommand.IsEmpty() == false) {
        if (m_bSinglePlayer) {
            ...
            if (pMatchMaker) { pMatchMaker->OnCommand(m_strPostPromptCommand); }
        }
    }
}
...
void CBasePanel::OnCommand(const char *command) {
    if (GameUI().IsConsoleUI()) {
        ...
        if (IsPromptableCommand(command)) {
            ...
            if (HandleSignInRequest(command) == false) return;

            // Handle storage
            if (HandleStorageDeviceRequest(command) == false) return;
...
void CBasePanel::RunAnimationWithCallback(vgui::Panel *parent, const char *animName, KeyValues *msgFunc) {
    if (!m_pConsoleAnimationController) return;
    ...
    if (sequenceLength) { sequenceLength += g_flAnimationPadding; }
    if (parent && msgFunc) { PostMessage(parent, msgFunc, sequenceLength); }
}
...
void CBasePanel::OnOpenNewGameDialog(const char *chapter) {
    ...
    if (chapter) { ((CNewGameDialog *)m_hNewGameDialog.Get())->SetSelectedChapter(chapter); }
...
//-----------------------------------------------------------------------------
// Purpose: forces any changed options dialog settings to be applied
// immediately, if it's open
//-----------------------------------------------------------------------------
void CBasePanel::ApplyOptionsDialogSettings() {
    if (m_hOptionsDialog.Get()) { m_hOptionsDialog->ApplyChanges(); }
}
...
#ifdef POSIX
    // Alfred says this is old legacy code that allowed you to walk through
    // looking for gameinfos and switch and it's not needed anymore. So I'm
    // killing this assert...
#else
...
void CBasePanel::OnOpenCSAchievementsDialog() {
    if (GameClientExports()) {
        ...
        // [smessick] For lower resolutions, open the Steam achievements instead of
        // the CSS achievements screen.
        if (screenWide < GameClientExports()->GetAchievementsPanelMinWidth()) {
            ISteamFriends *friends = steamapicontext->SteamFriends();
            if (friends) { friends->ActivateGameOverlay("Achievements"); }
        } else {
...
void CBasePanel::OnOpenMatchmakingBasePanel() {
    ...
    if (m_pGameLogo) { m_pGameLogo->SetVisible(false); }

    // Hide the standard game menu
    for (int i = 0; i < m_pGameMenuButtons.Count(); ++i) { m_pGameMenuButtons[i]->SetVisible(false); }
...
CMatchmakingBasePanel *CBasePanel::GetMatchmakingBasePanel() {
    ...
    if (m_bUseMatchmaking) { pBase = dynamic_cast<CMatchmakingBasePanel *>(m_hMatchmakingBasePanel.Get()); }
    return pBase;
}
...
void CBasePanel::SessionNotification(const int notification, const int param) {
    ...
    CMatchmakingBasePanel *pBase = GetMatchmakingBasePanel();
    if (pBase) { pBase->SessionNotification(notification, param); }
}
...
void CBasePanel::SystemNotification(const int notification) {
    CMatchmakingBasePanel *pBase = GetMatchmakingBasePanel();
    if (pBase) { pBase->SystemNotification(notification); }
    ...
    if (notification == SYSTEMNOTIFY_USER_SIGNEDIN) {
        ...
        uint state = XUserGetSigninState(XBX_GetPrimaryUserId());
        if (state != eXUserSigninState_NotSignedIn) { return; }
        ...
        // If we're waiting for the user to sign in, and check if they selected a
        // usable profile
...
void CBasePanel::UpdatePlayerInfo(uint64 nPlayerId, const char *pName, int nTeam, byte cVoiceState, int nPlayersNeeded, bool bHost) {
    CMatchmakingBasePanel *pBase = GetMatchmakingBasePanel();
    if (pBase) { pBase->UpdatePlayerInfo(nPlayerId, pName, nTeam, cVoiceState, nPlayersNeeded, bHost); }
}
...
void CBasePanel::SessionSearchResult(int searchIdx, void *pHostData, XSESSION_SEARCHRESULT *pResult, int ping) {
    CMatchmakingBasePanel *pBase = GetMatchmakingBasePanel();
    if (pBase) { pBase->SessionSearchResult(searchIdx, pHostData, pResult, ping); }
}
...
void CBasePanel::OnGameUIHidden() {
    if (m_hOptionsDialog.Get()) { PostMessage(m_hOptionsDialog.Get(), new KeyValues("GameUIHidden")); }
}
...
void CBasePanel::SetMenuAlpha(int alpha) {
    ...
    if (m_pGameLogo) { m_pGameLogo->SetAlpha(alpha); }

    for (int i = 0; i < m_pGameMenuButtons.Count(); ++i) { m_pGameMenuButtons[i]->SetAlpha(alpha); }
    m_bForceTitleTextUpdate = true;
}
...
void CBasePanel::SetMainMenuOverride(vgui::VPANEL panel) {
    ...
    if (m_pGameMenu) { m_pGameMenu->SetMainMenuOverride(panel); }
...
void CBasePanel::SetMenuItemBlinkingState(const char *itemName, bool state) {
    for (int i = 0; i < GetChildCount(); i++) {
        ...
        if (pGameMenu) { pGameMenu->SetMenuItemBlinkingState(itemName, state); }
    }
}
...
//-----------------------------------------------------------------------------
// Purpose: adds button icons and help text to the footer panel when activating
// a menu
//-----------------------------------------------------------------------------
void CFooterPanel::AddButtonsFromMap(vgui::Frame *pMenu) {
    ...
    if (pMap) {
        ...
        for (int i = 0; i < buttonCt; ++i) {
            ...
            if (pText) { AddNewButtonLabel(pText, pMap->GetBindingIcon(i)); }
        }
    }
}
...
void CFooterPanel::SetHelpNameAndReset(const char *pName) {
    ...
    if (pName) { m_pHelpName = strdup(pName); }
...
void CFooterPanel::Paint(void) {
    ...
    if (!m_bCenterHorizontal) {
        ...
        for (int i = 0; i < m_ButtonLabels.Count(); ++i) {
            ...
            // Draw the button
            // back up button width and a little extra to leave a gap between button
            // and text
        ...
        // need to loop through and figure out how wide our buttons and text are
        // (with gaps between) so we can offset from the center
...
void CMessageDialogHandler::ShowMessageDialog(int nType, vgui::Panel *pOwner) {
    int iSimpleFrame = 0;
    if (ModInfo().IsSinglePlayerOnly()) { iSimpleFrame = MD_SIMPLEFRAME; }

    switch (nType) {
    case MD_SEARCHING_FOR_GAMES:
        CreateMessageDialog(MD_CANCEL | MD_RESTRICTPAINT, NULL, "#TF_Dlg_SearchingForGames", NULL, "CancelOperation", pOwner, true);
        break;

    case MD_CREATING_GAME:
        CreateMessageDialog(MD_RESTRICTPAINT, NULL, "#TF_Dlg_CreatingGame", NULL, NULL, pOwner, true);
        break;

    case MD_SESSION_SEARCH_FAILED:
        CreateMessageDialog(MD_YESNO | MD_RESTRICTPAINT, NULL, "#TF_Dlg_NoGamesFound", "ShowSessionOptionsDialog", "ReturnToMainMenu", pOwner);
        break;

    case MD_SESSION_CREATE_FAILED:
        CreateMessageDialog(MD_OK, NULL, "#TF_Dlg_CreateFailed", "ReturnToMainMenu", NULL, pOwner);
        break;

    case MD_SESSION_CONNECTING:
        CreateMessageDialog(0, NULL, "#TF_Dlg_Connecting", NULL, NULL, pOwner, true);
        break;

    case MD_SESSION_CONNECT_NOTAVAILABLE:
        CreateMessageDialog(MD_OK, NULL, "#TF_Dlg_JoinRefused", "ReturnToMainMenu", NULL, pOwner);
        break;

    case MD_SESSION_CONNECT_SESSIONFULL:
        CreateMessageDialog(MD_OK, NULL, "#TF_Dlg_GameFull", "ReturnToMainMenu", NULL, pOwner);
        break;

    case MD_SESSION_CONNECT_FAILED:
        CreateMessageDialog(MD_OK, NULL, "#TF_Dlg_JoinFailed", "ReturnToMainMenu", NULL, pOwner);
        break;

    case MD_LOST_HOST:
        CreateMessageDialog(MD_OK | MD_RESTRICTPAINT, NULL, "#TF_Dlg_LostHost", "ReturnToMainMenu", NULL, pOwner);
        break;

    case MD_LOST_SERVER:
        CreateMessageDialog(MD_OK | MD_RESTRICTPAINT, NULL, "#TF_Dlg_LostServer", "ReturnToMainMenu", NULL, pOwner);
        break;

    case MD_MODIFYING_SESSION:
        CreateMessageDialog(MD_RESTRICTPAINT, NULL, "#TF_Dlg_ModifyingSession", NULL, NULL, pOwner, true);
        break;

    case MD_SAVE_BEFORE_QUIT:
        CreateMessageDialog(MD_YESNO | iSimpleFrame | MD_RESTRICTPAINT, "#GameUI_QuitConfirmationTitle", "#GameUI_Console_QuitWarning", "QuitNoConfirm",
                            "CloseQuitDialog_OpenMainMenu", pOwner);
        break;

    case MD_QUIT_CONFIRMATION:
        CreateMessageDialog(MD_YESNO | iSimpleFrame | MD_RESTRICTPAINT, "#GameUI_QuitConfirmationTitle", "#GameUI_QuitConfirmationText", "QuitNoConfirm",
                            "CloseQuitDialog_OpenMainMenu", pOwner);
        break;

    case MD_QUIT_CONFIRMATION_TF:
        CreateMessageDialog(MD_YESNO | MD_RESTRICTPAINT, "#GameUI_QuitConfirmationTitle", "#GameUI_QuitConfirmationText", "QuitNoConfirm",
                            "CloseQuitDialog_OpenMatchmakingMenu", pOwner);
        break;

    case MD_DISCONNECT_CONFIRMATION:
        CreateMessageDialog(MD_YESNO | MD_RESTRICTPAINT, "", "#GameUI_DisconnectConfirmationText", "DisconnectNoConfirm", "close_dialog", pOwner);
        break;

    case MD_DISCONNECT_CONFIRMATION_HOST:
        CreateMessageDialog(MD_YESNO | MD_RESTRICTPAINT, "", "#GameUI_DisconnectHostConfirmationText", "DisconnectNoConfirm", "close_dialog", pOwner);
        break;

    case MD_KICK_CONFIRMATION:
        CreateMessageDialog(MD_YESNO, "", "#TF_Dlg_ConfirmKick", "KickPlayer", "close_dialog", pOwner);
        break;

    case MD_CLIENT_KICKED:
        CreateMessageDialog(MD_OK | MD_RESTRICTPAINT, "", "#TF_Dlg_ClientKicked", "close_dialog", NULL, pOwner);
        break;

    case MD_EXIT_SESSION_CONFIRMATION:
        CreateMessageDialog(MD_YESNO, "", "#TF_Dlg_ExitSessionText", "ReturnToMainMenu", "close_dialog", pOwner);
        break;

    case MD_STORAGE_DEVICES_NEEDED:
        CreateMessageDialog(MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE | iSimpleFrame | MD_RESTRICTPAINT, "#GameUI_Console_StorageRemovedTitle",
                            "#GameUI_Console_StorageNeededBody", "ShowDeviceSelector", "QuitNoConfirm", pOwner);
        break;

    case MD_STORAGE_DEVICES_CHANGED:
        CreateMessageDialog(MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE | iSimpleFrame | MD_RESTRICTPAINT, "#GameUI_Console_StorageRemovedTitle",
                            "#GameUI_Console_StorageRemovedBody", "ShowDeviceSelector", "clear_storage_deviceID", pOwner);
        break;

    case MD_STORAGE_DEVICES_TOO_FULL:
        CreateMessageDialog(MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE | iSimpleFrame | MD_RESTRICTPAINT, "#GameUI_Console_StorageTooFullTitle",
                            "#GameUI_Console_StorageTooFullBody", "ShowDeviceSelector", "StorageDeviceDenied", pOwner);
        break;

    case MD_PROMPT_STORAGE_DEVICE:
        CreateMessageDialog(MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE | iSimpleFrame | MD_RESTRICTPAINT, "#GameUI_Console_NoStorageDeviceSelectedTitle",
                            "#GameUI_Console_NoStorageDeviceSelectedBody", "ShowDeviceSelector", "StorageDeviceDenied", pOwner);
        break;

    case MD_PROMPT_STORAGE_DEVICE_REQUIRED:
        CreateMessageDialog(MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE | MD_SIMPLEFRAME, "#GameUI_Console_NoStorageDeviceSelectedTitle",
                            "#GameUI_Console_StorageDeviceRequiredBody", "ShowDeviceSelector", "RequiredStorageDenied", pOwner);
        break;

    case MD_PROMPT_SIGNIN:
        CreateMessageDialog(MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE | iSimpleFrame, "#GameUI_Console_NoUserProfileSelectedTitle",
                            "#GameUI_Console_NoUserProfileSelectedBody", "ShowSignInUI", "SignInDenied", pOwner);
        break;

    case MD_PROMPT_SIGNIN_REQUIRED:
        CreateMessageDialog(MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE | iSimpleFrame, "#GameUI_Console_NoUserProfileSelectedTitle",
                            "#GameUI_Console_UserProfileRequiredBody", "ShowSignInUI", "RequiredSignInDenied", pOwner);
        break;

    case MD_NOT_ONLINE_ENABLED:
        CreateMessageDialog(MD_YESNO | MD_WARNING, "", "#TF_Dlg_NotOnlineEnabled", "ShowSigninUI", "close_dialog", pOwner);
        break;

    case MD_NOT_ONLINE_SIGNEDIN:
        CreateMessageDialog(MD_YESNO | MD_WARNING, "", "#TF_Dlg_NotOnlineSignedIn", "ShowSigninUI", "close_dialog", pOwner);
        break;

    case MD_DEFAULT_CONTROLS_CONFIRM:
        CreateMessageDialog(MD_YESNO | MD_WARNING | iSimpleFrame | MD_RESTRICTPAINT, "#GameUI_RestoreDefaults", "#GameUI_ControllerSettingsText",
                            "DefaultControls", "close_dialog", pOwner);
        break;

    case MD_AUTOSAVE_EXPLANATION:
        CreateMessageDialog(MD_OK | MD_WARNING | iSimpleFrame | MD_RESTRICTPAINT, "#GameUI_ConfirmNewGame_Title", "#GameUI_AutoSave_Console_Explanation",
                            "StartNewGameNoCommentaryExplanation", NULL, pOwner);
        break;

    case MD_COMMENTARY_EXPLANATION:
        CreateMessageDialog(MD_OK | MD_WARNING | iSimpleFrame | MD_RESTRICTPAINT, "#GameUI_CommentaryDialogTitle", "#GAMEUI_Commentary_Console_Explanation",
                            "StartNewGameNoCommentaryExplanation", NULL, pOwner);
        break;

    case MD_COMMENTARY_EXPLANATION_MULTI:
        CreateMessageDialog(MD_OK | MD_WARNING, "#GameUI_CommentaryDialogTitle", "#GAMEUI_Commentary_Console_Explanation",
                            "StartNewGameNoCommentaryExplanation", NULL, pOwner);
        break;

    case MD_COMMENTARY_CHAPTER_UNLOCK_EXPLANATION:
        CreateMessageDialog(MD_OK | MD_WARNING | iSimpleFrame | MD_RESTRICTPAINT, "#GameUI_CommentaryDialogTitle", "#GameUI_CommentaryUnlock", "close_dialog",
                            NULL, pOwner);
        break;

    case MD_SAVE_BEFORE_LANGUAGE_CHANGE:
        CreateMessageDialog(MD_YESNO | MD_WARNING | MD_SIMPLEFRAME | MD_COMMANDAFTERCLOSE | MD_RESTRICTPAINT, "#GameUI_ChangeLanguageRestart_Title",
                            "#GameUI_ChangeLanguageRestart_Info", "AcceptVocalsLanguageChange", "CancelVocalsLanguageChange", pOwner);

    case MD_SAVE_BEFORE_NEW_GAME:
        CreateMessageDialog(MD_OKCANCEL | MD_WARNING | iSimpleFrame | MD_COMMANDAFTERCLOSE | MD_RESTRICTPAINT, "#GameUI_ConfirmNewGame_Title",
                            "#GameUI_NewGameWarning", "StartNewGame", "close_dialog", pOwner);
        break;

    case MD_SAVE_BEFORE_LOAD:
        CreateMessageDialog(MD_OKCANCEL | MD_WARNING | iSimpleFrame | MD_COMMANDAFTERCLOSE | MD_RESTRICTPAINT, "#GameUI_ConfirmLoadGame_Title",
                            "#GameUI_LoadWarning", "LoadGame", "LoadGameCancelled", pOwner);
        break;

    case MD_DELETE_SAVE_CONFIRM:
        CreateMessageDialog(MD_OKCANCEL | MD_WARNING | iSimpleFrame | MD_COMMANDAFTERCLOSE, "#GameUI_ConfirmDeleteSaveGame_Title",
                            "#GameUI_ConfirmDeleteSaveGame_Info", "DeleteGame", "DeleteGameCancelled", pOwner);
        break;

    case MD_SAVE_OVERWRITE:
        CreateMessageDialog(MD_OKCANCEL | MD_WARNING | iSimpleFrame | MD_COMMANDAFTERCLOSE, "#GameUI_ConfirmOverwriteSaveGame_Title",
                            "#GameUI_ConfirmOverwriteSaveGame_Info", "SaveGame", "OverwriteGameCancelled", pOwner);
        break;

    case MD_SAVING_WARNING:
        CreateMessageDialog(MD_WARNING | iSimpleFrame | MD_COMMANDONFORCECLOSE, "", "#GameUI_SavingWarning", "SaveSuccess", NULL, pOwner, true);
        break;

    case MD_SAVE_COMPLETE:
        CreateMessageDialog(MD_OK | iSimpleFrame | MD_COMMANDAFTERCLOSE, "#GameUI_ConfirmOverwriteSaveGame_Title", "#GameUI_GameSaved", "CloseAndSelectResume",
                            NULL, pOwner);
        break;

    case MD_LOAD_FAILED_WARNING:
        CreateMessageDialog(MD_OK | MD_WARNING | iSimpleFrame, "#GameUI_LoadFailed", "#GameUI_LoadFailed_Description", "close_dialog", NULL, pOwner);
        break;

    case MD_OPTION_CHANGE_FROM_X360_DASHBOARD:
        CreateMessageDialog(MD_OK | iSimpleFrame | MD_RESTRICTPAINT, "#GameUI_SettingChangeFromX360Dashboard_Title",
                            "#GameUI_SettingChangeFromX360Dashboard_Info", "close_dialog", NULL, pOwner);
        break;

    case MD_STANDARD_SAMPLE:
        CreateMessageDialog(MD_OK, "Standard Dialog", "This is a standard dialog", "close_dialog", NULL, pOwner);
        break;

    case MD_WARNING_SAMPLE:
        CreateMessageDialog(MD_OK | MD_WARNING, "#GameUI_Dialog_Warning", "This is a warning dialog", "close_dialog", NULL, pOwner);
        break;

    case MD_ERROR_SAMPLE:
        CreateMessageDialog(MD_OK | MD_ERROR, "Error Dialog", "This is an error dialog", "close_dialog", NULL, pOwner);
        break;

    case MD_STORAGE_DEVICES_CORRUPT:
        CreateMessageDialog(MD_OK | MD_WARNING | iSimpleFrame | MD_RESTRICTPAINT, "", "#GameUI_Console_FileCorrupt", "close_dialog", NULL, pOwner);
        break;

    case MD_CHECKING_STORAGE_DEVICE:
        CreateMessageDialog(iSimpleFrame | MD_RESTRICTPAINT, NULL, "#GameUI_Dlg_CheckingStorageDevice", NULL, NULL, pOwner, true);
        break;
...
void CMessageDialogHandler::CloseAllMessageDialogs() {
    for (int i = 0; i < MAX_MESSAGE_DIALOGS; ++i) {
        ...
        if (pDlg) {
            vgui::surface()->RestrictPaintToSinglePanel(NULL);
            if (vgui_message_dialog_modal.GetBool()) { vgui::input()->ReleaseAppModalSurface(); }
...
void CMessageDialogHandler::CreateMessageDialog(const uint nType, const char *pTitle, const char *pMsg, const char *pCmdA, const char *pCmdB,
                                                vgui::Panel *pCreator, bool bShowActivity /*= false*/) {
    int nStackIdx = 0;
    if (nType & MD_WARNING) {
        nStackIdx = DIALOG_STACK_IDX_WARNING;
    } else if (nType & MD_ERROR) {
    ...
    if (nType & MD_RESTRICTPAINT) { vgui::surface()->RestrictPaintToSinglePanel(m_hMessageDialogs[nStackIdx]->GetVPanel()); }
...
void CMessageDialogHandler::ActivateMessageDialog(int nStackIdx) {
    ...
    for (idx; idx >= nStackIdx; --idx) {
        ...
        if (pDialog) {
            pDialog->Activate();
            if (vgui_message_dialog_modal.GetBool()) { vgui::input()->SetAppModalSurface(pDialog->GetVPanel()); }
...
void CMessageDialogHandler::PositionDialogs(int wide, int tall) {
    for (int i = 0; i < MAX_MESSAGE_DIALOGS; ++i) {
        if (m_hMessageDialogs[i].Get()) { PositionDialog(m_hMessageDialogs[i], wide, tall); }
    }
}
...
void CMainMenuGameLogo::ApplySchemeSettings(vgui::IScheme *pScheme) {
    ...
    if (pConditions) {
        ...
        if (pSubKey) { pConditions->AddSubKey(pSubKey); }
    }
...
void CBasePanel::CloseBaseDialogs(void) {
    if (m_hNewGameDialog.Get()) m_hNewGameDialog->Close();

    if (m_hAchievementsDialog.Get()) m_hAchievementsDialog->Close();

    if (m_hBonusMapsDialog.Get()) m_hBonusMapsDialog->Close();

    if (m_hControllerDialog.Get()) m_hControllerDialog->Close();

    if (m_hLoadGameDialog_Xbox.Get()) m_hLoadGameDialog_Xbox->Close();

    if (m_hOptionsDialog_Xbox.Get()) m_hOptionsDialog_Xbox->Close();

    if (m_hSaveGameDialog_Xbox.Get()) m_hSaveGameDialog_Xbox->Close();

    if (m_hLoadCommentaryDialog.Get()) m_hLoadCommentaryDialog->Close();

    if (m_hCreateMultiplayerGameDialog.Get()) m_hCreateMultiplayerGameDialog->Close();
}
...
static void RefreshOptionsDialog(const CCommand &args) {
    if (g_hOptionsDialog) {
        ...
        if (pBasePanel) {
            ...
            if (pOptionsDialog) { pOptionsDialog->GetPropertySheet()->SetActivePage(pOptionsDialog->GetOptionsSubMultiplayer()); }
        }
    }
}
